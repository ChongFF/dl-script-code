...
REM conda install -y conda-build
CALL activate machinelearning

ECHO ### Try Jupyter Notebook after this ###
conda install -y jupyter
conda install -y numpy matplotlib scipy scikit-learn pillow pandas jupyter cython mingw libpython

ECHO #####################################
ECHO # install Machine Learning packages #
ECHO #####################################
pip install PyHamcrest imagehash hyperopt kaggle 
ECHO #PyHamcrest is required for kaggle
...
conda install -y -c r r-base64enc=0.1_3=mro343h889e2dd_0
